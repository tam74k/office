...
export const CitiesView: React.FC<CitiesViewProps> = ({
  ...
  return (
    <div id="cities-view-root" className="space-y-6 pb-12">
      ...
      <div className="bg-white dark:bg-slate-900 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs grid grid-cols-1 sm:grid-cols-2 gap-3">
        {/* Country Selector Filter */}
        <div>
          <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1">تصفية حسب الدولة:</label>
          <select
            value={selectedCountryId}
            onChange={(e) => setSelectedCountryId(e.target.value)}
            className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-950/50 font-medium"
          >
        ...
        {/* Search */}
        <div>
          <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1">بحث عن اسم المدينة:</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {filteredCities.map(city => {
          ...
          return (
            <div
              key={city.id}
              className="bg-white dark:bg-slate-900 dark:bg-slate-950 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs hover:border-emerald-300 transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-1 mb-1.5">
                  <div className="flex items-center gap-1.5 overflow-hidden">
                    <span className="text-xl shrink-0">{country?.flag_emoji || '📍'}</span>
                    <h3 className="font-bold text-xs text-slate-900 dark:text-white truncate">{city.name}</h3>
                  </div>

                  <div className="flex items-center gap-0.5 shrink-0">
                    <button
                      onClick={() => openEditModal(city)}
                      className="p-1 text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md"
                    >
                    ...
                    <button
                      ...
                      className="p-1 text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-md"
                    >
                ...
                <div className="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium truncate">
                  {country?.name || 'دولة'}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white dark:bg-slate-900 dark:bg-slate-950 rounded-2xl shadow-2xl max-w-md w-full border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-200 mb-1">الدولة التابعة لها المدينة:</label>
                <select
                  value={formData.country_id || ''}
                  onChange={(e) => setFormData(p => ({ ...p, country_id: e.target.value }))}
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-950/50 font-semibold"
                  required
                >
                  {countries.map(c => (
                    <option key={c.id} value={c.id}>
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-200 mb-1">اسم المدينة:</label>
                <input
                  type="text"
                  value={formData.name || ''}
