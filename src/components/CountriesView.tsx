      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
              filterType === 'sponsor' ? 'bg-emerald-700 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
              filterType === 'worker' ? 'bg-blue-700 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            ...
            <div>
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400 font-mono font-semibold uppercase">{country.code}</span>
                  </div>
                </div>
              ...
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 space-y-1 text-xs">
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-[11px]">كود الاتصال الدولي:</span>
                  <span className="font-mono font-bold text-emerald-800 dir-ltr">{country.phone_code}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-[11px]">تصنيف الدولة:</span>
                  {country.is_sponsor_country ? (
                    <span className="text-emerald-800 font-bold text-[11px] bg-emerald-100 px-2 py-0.5 rounded-md">
                      دولة كفيل (مستقدم)
                    </span>
                  ) : (
                    <span className="text-blue-800 font-bold text-[11px] bg-blue-100 px-2 py-0.5 rounded-md">
                      دولة عامل (استقدام)
                    </span>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-slate-200 text-slate-800">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">اسم الدولة:</label>
                <input
                  type="text"
                  value={formData.name || ''}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 mb-1">كود الاتصال الدولي:</label>
                  <input
                    type="text"
                    value={formData.phone_code || ''}
                    onChange={(e) => setFormData(p => ({ ...p, phone_code: e.target.value }))}
                    placeholder="+966"
                    className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-slate-50 font-mono dir-ltr text-right"
                    required
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-slate-700 mb-1">رمز العلم (Emoji):</label>
                  <input
                    type="text"
                    value={formData.flag_emoji || ''}
                    onChange={(e) => setFormData(p => ({ ...p, flag_emoji: e.target.value }))}
                    placeholder="🇸🇦"
                    className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-slate-50 text-center text-lg"
                  />
                </div>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">رمز الدولة (ISO Code):</label>
                <input
                  type="text"
                  value={formData.code || ''}
                  onChange={(e) => setFormData(p => ({ ...p, code: e.target.value }))}
                  placeholder="SA"
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-slate-50 font-mono uppercase"
                />
              </div>
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <label className="flex items-center gap-2.5 cursor-pointer">
                  ...
                  />
                  <div>
                    <span className="font-bold text-slate-900 block text-xs">تعيين كـ (دولة كفيل / مستقدم)</span>
