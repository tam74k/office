    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">تصفية وطباعة تقارير الطلبات التفصيلية</p>
        </div>
        <button
          onClick={handlePrint}
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm print:hidden space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
          ...
          <h4 className="font-bold text-sm text-slate-900">عوامل التصفية</h4>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-2 py-2 text-[10px] sm:text-xs rounded-xl border border-slate-300 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none bg-slate-50 font-mono"
            />
          </div>
      ...
      <div className="hidden print:flex flex-row items-start justify-between border-b-2 border-slate-800 pb-4 mb-6">
        <div className="text-right">
          <h1 className="text-2xl font-bold text-slate-900 mb-1">{officeProfile.name || 'مكتب الاستقدام'}</h1>
          {officeProfile.cr_number && <p className="text-sm font-bold text-slate-700">رقم السجل التجاري: <span className="font-mono">{officeProfile.cr_number}</span></p>}
          {officeProfile.license_number && <p className="text-sm font-bold text-slate-700">ترخيص رقم: <span className="font-mono">{officeProfile.license_number}</span></p>}
        </div>
        <div className="text-left flex flex-col items-end justify-start">
          <div className="w-20 h-20 rounded-2xl bg-slate-100 border-2 border-slate-300 flex items-center justify-center text-slate-400 font-bold text-xl overflow-hidden">
            شعار المكتب
          </div>
        </div>
      </div>
      ...
      <div className="hidden print:block text-center mb-6">
        <h2 className="text-xl font-bold text-slate-900 border-b-2 border-slate-300 inline-block pb-1 px-4">تقرير الطلبات التفصيلي</h2>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden print:shadow-none print:border-none print:rounded-none">
        ...
        {filteredOrders.length === 0 ? (
          <div className="p-12 text-center text-slate-400 print:hidden">
            <p className="text-sm font-medium text-slate-600">لا توجد نتائج مطابقة</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-right text-xs print:text-[10px]">
              <thead className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 print:bg-slate-200 print:border-slate-400 print:text-black">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100 print:divide-slate-300">
                {filteredOrders.map(order => (
                  <tr key={order.id} className="hover:bg-slate-50/80 transition-colors print:hover:bg-transparent">
                    <td className="p-3 print:p-2 font-mono font-bold text-slate-900 print:text-black">{order.order_number}</td>
                    <td className="p-3 print:p-2 font-mono text-slate-600 print:text-black">{formatDate(order.contract_date || order.created_at)}</td>
                    <td className="p-3 print:p-2 font-bold text-slate-900 print:text-black">{order.client_name}</td>
                    <td className="p-3 print:p-2 font-mono text-slate-600 print:text-black" dir="ltr">{order.client_mobile}</td>
                    <td className="p-3 print:p-2">
                      <div className="space-y-1">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-1">
                            <span className="font-bold text-slate-800 print:text-black">{item.profession_name}</span>
                            <span className="text-slate-400 print:text-black">من</span>
                            <span className="font-bold text-emerald-700 print:text-black">{item.worker_country_name}</span>
                          </div>
                        ))}
                      </div>
                    </td>
                    <td className="p-3 print:p-2 font-bold text-slate-700 print:text-black">{order.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="hidden print:block mt-8 pt-4 border-t-2 border-slate-800 text-center space-y-2">
        {officeProfile.address && <p className="text-sm text-slate-800 font-bold">{officeProfile.address} {officeProfile.city && ` - ${officeProfile.city}`}</p>}
        <div className="flex justify-center gap-6 font-mono font-bold text-sm text-slate-800" dir="ltr">
