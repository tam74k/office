    ...
    <div id="orders-view-root" className="space-y-6 pb-12">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="relative">
          <label className="block text-[11px] font-bold text-slate-600 mb-1">بحث شامل:</label>
          <div className="relative">
            <input
        ...
        {/* Status filter */}
        <div>
          <label className="block text-[11px] font-bold text-slate-600 mb-1">تصفية بحالة الطلب:</label>
          <select
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400">
            ...
            <p className="font-bold text-slate-700">لا توجد طلبات مطابقة</p>
            <p className="text-xs text-slate-500 mt-1">جرب تغيير شروط البحث أو اضغط على إنشاء طلب جديد</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="p-4 sm:p-5 bg-slate-50/80 border-b border-slate-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div className="flex items-start sm:items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold font-mono text-sm shrink-0">
                      #{order.order_number.replace('REC-', '')}
                    </div>
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="font-bold text-sm text-slate-900">{order.client_name}</h3>
                        <span className="text-xs text-slate-500 font-mono dir-ltr">({order.client_mobile})</span>
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 mt-1">
                        <span className="font-mono text-slate-600 font-semibold">{order.order_number}</span>
                        <span>•</span>
                  ...
                  <div className="flex flex-wrap items-center gap-3 justify-between lg:justify-end">
                    <div className="text-left font-mono text-xs bg-white px-3 py-1.5 rounded-xl border border-slate-200">
                      <div className="text-slate-500 text-[10px]">إجمالي العقد / المتبقي:</div>
                      <span className="font-bold text-slate-900">{formatCurrency(order.total_cost, officeProfile.default_currency)}</span>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="p-2 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 text-slate-700 rounded-xl text-xs font-medium transition-colors"
                      >
                      ...
                      <button
                        ...
                        title="حذف الطلب"
                        className="p-2 bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-700 rounded-xl text-xs font-medium transition-colors"
                      >
                ...
                <div className="p-4 sm:p-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {order.items.map((item, idx) => (
                      <div 
                        key={item.id || idx} 
                        className="bg-slate-50 rounded-xl p-3.5 border border-slate-200/90 hover:border-emerald-300 transition-colors flex flex-col justify-between"
                      >
                        <div>
                          <div className="flex items-start justify-between gap-2 mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <h4 className="font-bold text-xs text-slate-900">{item.profession_name}</h4>
                            ...
                            <div className="relative inline-block">
                              <select
                                ...
                                className="appearance-none px-2.5 py-1 pr-2 pl-6 rounded-lg text-[11px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 cursor-pointer focus:outline-hidden"
                              >
                          ...
                          {/* Requirements & specs */}
                          <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600 bg-white p-2 rounded-lg border border-slate-100 mb-2">
                          ...
                          {item.candidate_name && (
                            <div className="text-[11px] text-slate-700 mb-2 bg-emerald-50/70 p-2 rounded-lg border border-emerald-200/60">
                              <span className="text-slate-500">المرشح المختار:</span> <strong>{item.candidate_name}</strong>
                              {item.passport_number && <span className="mr-2 text-[10px] text-slate-500 font-mono">(جواز: {item.passport_number})</span>}
                            </div>
                          )}
                        ...
                        <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-[11px]">
                          <span className="text-slate-500">تكلفة الاستقدام: <strong className="text-slate-900 font-mono">{formatCurrency(item.recruitment_cost, officeProfile.default_currency)}</strong></span>
                          <button
                            id={`btn-wa-item-${item.id}`}
                            onClick={() => handleSendWhatsApp(order, item)}
                            className="flex items-center gap-1 text-emerald-700 hover:text-emerald-800 font-bold bg-emerald-50 hover:bg-emerald-100 px-2 py-1 rounded-lg transition-colors"
                          >
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-200 text-slate-800">
            ...
            <div className="p-6 space-y-5 text-xs">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div>
                  <span className="text-slate-500 block text-[11px]">العميل:</span>
                  <span className="font-bold text-slate-900 text-sm">{selectedOrderForView.client_name}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[11px]">رقم الجوال:</span>
                  <span className="font-mono font-bold text-slate-900 dir-ltr text-right inline-block">{selectedOrderForView.client_mobile}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[11px]">الموقع:</span>
                  <span className="font-semibold text-slate-900">{selectedOrderForView.sponsor_country_name} - {selectedOrderForView.city_name}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[11px]">تاريخ العقد:</span>
                  <span className="font-medium text-slate-900">{formatDate(selectedOrderForView.contract_date)}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[11px]">الوصول المتوقع:</span>
                  <span className="font-medium text-slate-900">{formatDate(selectedOrderForView.expected_arrival_date) || 'غير محدد'}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[11px]">الحالة العامة:</span>
                  <span className="font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full inline-block mt-0.5">{selectedOrderForView.status}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedOrderForView.items.map((item, idx) => (
                    <div key={item.id || idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="font-bold text-sm text-slate-900 flex items-center gap-2">
                          ...
                          <span className="text-slate-400 font-normal">من دولة</span>
                      ...
                      {item.candidate_name && (
                        <div className="text-xs bg-emerald-50 p-2 rounded-lg text-emerald-900 border border-emerald-200">
              ...
              {/* Financial Box */}
              <div className="bg-emerald-50/60 p-4 rounded-xl border border-emerald-200 flex justify-between items-center text-xs">
                <div>
                  <span className="text-slate-500">إجمالي المبلغ:</span> <strong className="text-slate-900 font-mono text-sm">{formatCurrency(selectedOrderForView.total_cost, officeProfile.default_currency)}</strong>
                </div>
                <div>
                  <span className="text-slate-500">المدفوع:</span> <strong className="text-emerald-800 font-mono text-sm">{formatCurrency(selectedOrderForView.paid_amount, officeProfile.default_currency)}</strong>
                </div>
                <div>
                  <span className="text-slate-500">المتبقي:</span> <strong className="text-rose-700 font-mono text-sm font-black">{formatCurrency(selectedOrderForView.remaining_amount, officeProfile.default_currency)}</strong>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold transition-colors"
                >
                  <Printer className="w-4 h-4" />
      ...
      {isFormOpen && (
        <div 
          ...
          className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4"
        >
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[92vh] overflow-y-auto border border-slate-200 text-slate-800">
            {/* Modal Header */}
            <div className="bg-slate-900 text-white p-5 flex items-center justify-between sticky top-0 z-20">
              <div className="flex items-center gap-2">
                <FileText className="w-5 h-5 text-emerald-400" />
            ...
            <form onSubmit={handleSubmitOrderForm} className="p-6 space-y-6 text-xs">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-600 mb-1">رقم الطلب:</label>
                      <input
                        type="text"
                        value={formData.order_number || ''}
                        onChange={(e) => setFormData(p => ({ ...p, order_number: e.target.value }))}
                        className="w-full px-2.5 py-2 rounded-xl border border-slate-300 bg-white font-mono"
                        required
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-600 mb-1">تاريخ العقد:</label>
                      <input
                        type="date"
                ...
                {formData.client_id && (
                  <div className="p-2.5 bg-emerald-50 rounded-lg border border-emerald-200 text-slate-700 flex flex-wrap items-center justify-between text-[11px]">
                    <div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                ...
                {formData.items && formData.items.length > 0 && (
                  <div className="space-y-2">
                    {formData.items.map((it, idx) => (
                      <div key={it.id || idx} className="bg-white p-3 rounded-xl border border-slate-200 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <span className="font-bold text-slate-900">{it.profession_name}</span>
                            <span className="text-slate-400 mx-1.5">•</span>
                            <span className="text-emerald-700 font-semibold">{it.worker_country_name}</span>
                            <span className="text-slate-400 mx-1.5">•</span>
                            <span className="text-slate-600">{it.experience_type} ({it.age_min}-{it.age_max} سنة)</span>
                            <span className="text-slate-400 mx-1.5">•</span>
                ...
                <div className="p-3.5 bg-white rounded-xl border border-dashed border-emerald-300 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-600 mb-0.5">العمر من:</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-slate-600 mb-0.5">إلى سن:</label>
                        <input
                          type="number"
                    ...
                    {/* Salary */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 mb-0.5">الراتب المتفق عليه:</label>
                      <input
                        type="number"
                        value={currentItem.salary || 1500}
                        onChange={(e) => setCurrentItem(p => ({ ...p, salary: Number(e.target.value) }))}
                        className="w-full px-2 py-1.5 rounded-lg border border-slate-300 bg-slate-50 text-xs font-mono"
                      />
                    </div>

                    {/* Recruitment item cost */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 mb-0.5">تكلفة استقدام هذا البند:</label>
                      <input
                        type="number"
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1">إجمالي تكلفة العقد:</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1">الدفعة المدفوعة:</label>
                  <input
                    type="number"
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-white font-mono font-bold text-emerald-700"
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1">المتبقي عند الوصول:</label>
                  <input
                    type="number"
                    value={formData.remaining_amount || 0}
                    readOnly
                    className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-slate-100 font-mono font-bold text-rose-600"
                  />
                </div>
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 mb-1">ملاحظات وشروط خاصة:</label>
                <textarea
